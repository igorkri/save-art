...
class MyProjectController extends Controller
{
    ...
    /**
     * Отримати завершені проєкти (обидва джерела: save-art та art-ua-info)
     *
     * @OA\Get(
     *     path="/v1/my/projects/completed",
     *     operationId="getMyCompletedProjects",
     *     tags={"My Projects"},
    ...
    {
        $query = Project::query()
            ->whereIn('source', [ProjectSource::SaveArt, ProjectSource::ArtUaInfo])
            ->with(['user.profileLegal', 'projectParameters.parameter', 'projectParameters.parameterValue'])
            ->where('user_id', $request->user()->id)
